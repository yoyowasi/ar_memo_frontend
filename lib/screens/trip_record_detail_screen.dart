  ...
  void _deleteRecord(BuildContext context, WidgetRef ref) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('정말로 이 일기를 삭제하시겠습니까?\n삭제된 내용은 복구할 수 없습니다.', textAlign: TextAlign.center, style: bodyText2),
        actionsAlignment: MainAxisAlignment.center,
        actions: [
          TextButton(child: const Text('취소', style: TextStyle(color: subTextColor)), onPressed: () => Navigator.of(ctx).pop()),
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: const Text('삭제'),
            onPressed: () async {
              ...
              try {
                ...
                messenger.showSnackBar(const SnackBar(content: Text('일기가 삭제되었습니다.'), duration: Duration(seconds: 2)));
              } catch (e) {
                navigator.pop();
                messenger.showSnackBar(SnackBar(content: Text('삭제 실패: ${e.toString()}')));
              }
            },
          ),
        ],
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: recordAsync.when(
        data: (record) => RefreshIndicator( // 새로고침 기능 추가
                  ...
                  delegate: SliverChildListDelegate([
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      ...
                      if (record.group != null) Row(children: [
                        Icon(Icons.group, color: Color(record.group!.colorValue), size: 16), const SizedBox(width: 4),
                        Text(record.group!.name, style: bodyText1.copyWith(color: Color(record.group!.colorValue), fontSize: 14)),
                      ],),
                    ],),
